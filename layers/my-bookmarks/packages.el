...
(defconst my-bookmarks-packages '(bm helm-bm))

(defun my-bookmarks/post-init-bm ()
  (setq bm-highlight-style 'bm-highlight-only-fringe)

  ;; Customize the bm transient state.
  (add-hook 'spacemacs-post-user-config-hook
            #'my-bookmarks//customize-transient-state
            'append)
  ...
  (advice-add 'spacemacs/bm-transient-state/body
              :before #'my-bookmarks//load-repository)
...
(defun my-bookmarks/init-helm-bm ()
  (use-package helm-bm
    :defer t))
